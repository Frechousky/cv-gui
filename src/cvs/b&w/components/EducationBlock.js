...
export default class EducationBlock extends LeftPanelBlock {
  ...
  // overload
  renderInnerBlock() {
    const educationList = this.props.studies.map(
        (education, i) => <EducationItem key={i} education={education} />);
    return (
      <div>
        {educationList}
      </div>
  ...
  return (
    <div className="row education-item">
      <div className="col-12 education-item-title">
        <a {...urlAttributes}>
          {titleize(props.education.degree)} in {titleize(props.education.field)}<br />
